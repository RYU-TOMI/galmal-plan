...
A = {
    # 일본
    "NRT": ["야경", "골목", "쇼핑", "미식", "도시"], "HND": ["야경", "골목", "쇼핑", "미식", "도시"],
    "KIX": ["길거리음식", "고성", "쇼핑", "미식", "도시", "문화"], "ITM": ["미식", "길거리음식", "도시"],
    "FUK": ["길거리음식", "온천", "미식", "도시"],
    "OKA": ["해변", "리조트", "고성", "문화", "자연"],
    "CTS": ["자연", "미식", "온천", "도시"],
    "NGO": ["도시", "쇼핑", "문화", "고성"],
    ...
    "CJU": ["해변", "자연", "화산"],
    # 동남아
    "BKK": ["야시장", "사원", "쇼핑", "미식", "문화", "도시"], "DMK": ["야시장", "사원", "쇼핑", "미식", "문화", "도시"],
    "HKT": ["해변", "리조트", "야시장", "미식"],
    "CNX": ["문화", "사원", "자연", "미식", "야시장"],
    "DAD": ["리조트", "미식", "해변"],
    ...
    "DPS": ["해변", "서핑", "사원", "문화", "리조트"],
    "JKT": ["도시", "쇼핑"],   # CGK는 같은 자카르타라 뺐다(백엔드 T8에서 dests.py도 정리)
    "REP": ["문화", "유적", "사원"],
    "PNH": ["유적", "문화", "도시"],
    "VTE": ["문화", "사원", "자연"],
    "RGN": ["사원", "문화", "유적"],
    # ── 2026-09-02 추가 (직항 필터 249→16 중 6곳 채택) ──
    "TAG": ["해변", "리조트", "스노클링", "섬"],
    "RMQ": ["도시", "미식", "야시장"],
    "BKI": ["해변", "리조트", "자연", "섬"],
    "ISG": ["해변", "스노클링", "섬"],
...
# ---- 신규 추가 (2026-09-02 확정, 백엔드 T6에서 dests.py에 들어간다) ----
# dests.py에 아직 없으므로 메타데이터를 여기 들고 있는다.
# 좌표·haul은 백엔드가 벤더 공식 참조로 확인해 준 값이다.
NEW = {
    "TAG": ("보홀", "필리핀", "island", "mid"),
    "RMQ": ("타이중", "대만", "cn", "short"),
    "BKI": ("코타키나발루", "말레이시아", "sea", "mid"),
    "ISG": ("이시가키", "일본", "jp", "short"),
    "UBN": ("울란바토르", "몽골", "etc", "short"),
    "KLO": ("칼리보(보라카이)", "필리핀", "island", "mid"),
}
# ---- 삭제 예정 (백엔드 T8) ----
# CGK는 JKT와 같은 자카르타다. BB15에서 도시코드를 정규화할 때 빠졌다.
DROP = {"CGK"}
...
for _c in DROP:
    CUR.pop(_c, None)
for _c, (_ko, _country, _region, _haul) in NEW.items():
    CUR.setdefault(_c, (_ko, _country, _region, _haul, []))
...
def card_tags(tags):
    """카드 표시: **하위 태그 전부 + 상위 1개**(하위가 없으면 상위 2개), 최대 4개.

    개수를 고정하지 않는다 — **태그 개수 자체가 "즐길 게 얼마나 많은가"를 말한다.**
    싼야·프랑크푸르트는 2개, 두바이·발리·홍콩은 4개가 된다.
    하위를 앞세우는 이유: 상위(`도시`·`해변`)는 여러 도시가 공유해 구별에 쓸모가 없다.
    무엇을 앞세울지는 **하위끼리의 순서**로 통제한다.
    """
    subs = [t for t in tags if t in SUB]
    tops = [t for t in tags if t in TOP]
    out = subs + tops[:1] if subs else tops[:2]
    return out[:4]
...
L.append("## 배정표 (84개)")
L.append("")
L.append("`카드`는 **하위 전부 + 상위 1개**(하위 없으면 상위 2개), 최대 4개다. **개수가 곧 신호다** — 즐길 게 많은 도시는 태그도 많다.")
L.append("")
