...
- **feed_map.html** ⚠️ **낡음(2026-08-01)** — 오늘(08-22) 바꾼 것이 하나도 반영돼 있지 않다.
  `＋ 동남아`·`특가 N%↓` 도장·`휴양` 태그 등 폐기된 것들이 남아 있다. **참조하지 말 것.**
  ~~(최신 확정 홈 레이아웃)~~ — 지도(무대) + 좌 "오늘의 발견" 카드 피드 + 핀 플로팅 카드.
...
- **wireframe.html** ⭐ (PH5 진행 중) — **구조 재설계.** 색·사진 없이 구조만 본다.
  ① 무엇이 결정이고 무엇이 상속인지 감사표 ② 데스크톱 현재 구조 ③ 모바일 후보 2안 ④ 정할 것.
  지도는 실제 해안선을 회색조로 그린다. `python design/build_wireframe.py`로 재생성.
...
- `build_mobile_feed.py` → `mobile_feed.html` — **PH5b(2026-09-21)**. 폰에서 직접 여는 목업. A(피드만) / B(피드 + 전체 화면 지도)를 한 화면에서 바꿔 본다. 다른 목업과 달리 **틀을 그린 그림이 아니라 실제로 밀고 누르는 화면**이다.
