...
✅ **확정 (2026-08-22, PH4).** 위 방향대로 간다. 상세 규칙은 `SPEC.md` §CH4.
- 화면0 = 해시 없음, 출발지 = `#{허브}`, 딜 상세 = `#{허브}-{목적지}`. 전부 `pushState`.
- **키가 유일함을 확인했다** — `(허브, 목적지)` 조합이 딜 1건에 1:1(99/99, 중복 0).
- 딥링크로 들어오면 **거리 단계를 딜의 `haul`에 맞춰 자동 전환**한다(안 그러면 핀이 화면 밖).
- **공유 버튼**을 확장 상세에 둔다 — 딥링크만 있고 공유 수단이 없으면 반쪽이다.
...
- 노선 페이지 26개에는 **`{노선} 특가 알림 받기` 섹션과 mailto CTA가 살아 있다**(`site/route.py:220`).
...
| ~~IA-2~~ | ~~URL 해시 상태 · 딜 딥링크 · 사라진 딜 처리~~ | PH4 | ✅ **확정** (2026-08-22) |
